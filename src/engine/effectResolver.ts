...
export function resolveEffect(spec: EffectSpec, ctx: EffectContext): void {
  ...
  switch (spec.op) {
    ...
    case "HEAL_NEXUS": {
      ...
      if (amount > 0) {
        // D1 (teardown): cap at the player's OWN starting Hex (maxNexusHealth —
        // e.g. the 25-Hex newcomer cushion) and never reduce; the old hard 20
        // clamp made heals DAMAGE a player whose match started above 20.
        const hp = state.players[controller];
        const cur = hp.nexusHealth ?? STARTING_NEXUS_HEALTH;
        const cap = Math.max(hp.maxNexusHealth ?? STARTING_NEXUS_HEALTH, cur);
        hp.nexusHealth = Math.min(cap, cur + amount);
      }
      break;
    }
    case "DESTROY_UNIT": {
