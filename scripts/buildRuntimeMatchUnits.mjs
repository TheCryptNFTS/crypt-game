...
async function main() {
  ...
  // Compact tuple:
  // [id, cost, attack, health, speed, armor, keywords]
  ...
  await fs.writeFile(
    path.join(root, "src/data/runtimeMatchUnits.json"),
    JSON.stringify(rows),
    "utf8"
  );

  console.log(`Built runtimeMatchUnits.json with ${rows.length} rows`);
}
